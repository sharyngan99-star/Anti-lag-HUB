...
T.Text, T.TextColor3, T.TextSize, T.Font = "ANTI-LAG GUI", Color3.fromRGB(0,255,150), 15, Enum.Font.SourceSansBold
...
if scale >= 0.98 then Stat.Text = "Status: Native" ; pcall(function() Cam.FieldOfView = Cam.FieldOfView + 0.01 ; task.wait() ; Cam.FieldOfView = Cam.FieldOfView - 0.01 end) ; return end
local d = 1 / scale ; _G.ZLoop = RS.RenderStepped:Connect(function() if Cam then Cam.CFrame = Cam.CFrame * CFrame.new(0,0,0, d,0,0, 0,1,0, 0,0,1) end end)
Stat.Text = string.format("Status: Active\nFPS Boost: +%d%%", math.floor((1 - scale) * 100))
end
local function move(i)
local w, l = ST.AbsoluteSize.X, ST.AbsolutePosition.X ; if w <= 0 then return end
local p = math.clamp((i.Position.X - l) / w, 0, 1) ; scale = 0.25 + (p * 0.75)
SB.Position = UDim2.new(p, -10, 0, -7) ; SL.Text = "Render Quality: " .. math.floor(p * 100) .. "%" ; up()
end
TxtBtn.MouseButton1Click:Connect(function()
pcall(function()
settings().Rendering.QualityLevel = 1 ; local c = 0
for _, o in pairs(workspace:GetDescendants()) do
if o:IsA("Texture") or o:IsA("Decal") then o:Destroy() ; c = c + 1
elseif o:IsA("BasePart") and not o:IsA("MeshPart") then o.Material = Enum.Material.SmoothPlastic
elseif o:IsA("ParticleEmitter") or o:IsA("Trail") then o.Enabled = false end
end
TxtBtn.Text, TxtBtn.BackgroundColor3 = "Textures Cleared!", Color3.fromRGB(0,120,50) ; Stat.Text = "Removed: " .. c
end)
end)
RstBtn.MouseButton1Click:Connect(function() scale = 1.0 ; SB.Position = UDim2.new(1,-10,0,-7) ; SL.Text = "Render Quality: 100%" ; up() ; pcall(gcinfo) end)
local sfps, fcon = false, nil
FpsBtn.MouseButton1Click:Connect(function()
sfps = not sfps
if sfps then
FpsBtn.Text, FpsBtn.BackgroundColor3, FpsBtn.TextColor3 = "Show FPS: ON", Color3.fromRGB(25,50,25), Color3.fromRGB(100,255,100) ; FL.Visible = true
local lt, f = os.clock(), 0
fcon = RS.RenderStepped:Connect(function()
f = f + 1 ; local ct = os.clock()
if ct - lt >= 0.5 then local fps = math.floor(f / (ct - lt)) ; FL.Text = "FPS: " .. fps
FL.TextColor3 = fps >= 55 and Color3.fromRGB(0,255,150) or fps >= 30 and Color3.fromRGB(255,200,0) or Color3.fromRGB(255,50,50) ; f, lt = 0, ct end
end)
else
FpsBtn.Text, FpsBtn.BackgroundColor3, FpsBtn.TextColor3 = "Show FPS: OFF", Color3.fromRGB(45,25,25), Color3.fromRGB(255,100,100) ; FL.Visible = false
...
UIS.InputChanged:Connect(function(i) if drag and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then move(i) end end)
btn.MouseButton1Down:Connect(function() MF.Visible = not MF.Visible end)
task.spawn(function() while task.wait(20) do pcall(gcinfo) end end)
